# nf-core/parseseq: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/parseseq/usage](https://nf-co.re/parseseq/usage)
...
```console
nextflow run nf-core/parseseq --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/parseseq
```
...
First, go to the [nf-core/parseseq releases page](https://github.com/nf-core/parseseq/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
